...
- **Per-stage environment (18 Jul):** `Arenas/Arena` (stage 1 = grass meadow) vs `Arenas/Arena_Stage2` (stage 2 = snow), each self-contained (own DeploymentRing/Ground/props). New `StageEnvironmentView` (on the `Arenas` root, view-only) activates element `stageIndex % count`: applies on load from `WaveScheduler.PendingStageIndex` (resume boots into the right biome), re-applies on every `WaveStarted`, and PRE-SWAPS on a stage-complete `CheckpointReached` so the modal hides the switch — Continue reveals the new arena.
- **Manual environment toggle (18 Jul):** one circular button in the HUD header top-right (`HUD/TopBar/EnvToggleButton`, `EnvironmentToggleButton` script) — tap to flip to the next environment instantly and swap the button's own biome chip icon (grass ↔ snow, `UI/EnvIcons/Icon_EnvGrass|Snow.png`). It calls `StageEnvironmentView.CycleManual()`. Auto/manual coexist cleanly: `StageEnvironmentView.AutoApply` now only acts when the STAGE index actually CHANGES (tracked in `_autoStage`), so a manual pick is NOT snapped back on the next same-stage `WaveStarted` — it sticks until the next real stage transition re-asserts that stage's environment. The view raises `Changed(index)` on every swap (manual or auto) and the button subscribes, so the icon always matches the shown biome even on automatic stage swaps.
...
- **5.4** Log rolls `rollDistance` (9 tiles) from the DROP POINT along a **FIXED forward direction** (`LogEffectSO.rollDirection`, default world +Z = "up the screen" for the pitched-down, zero-yaw camera), then vanishes; hits each ground enemy in its lane once, knockback 1.2 (interrupts wind-ups; heavies half; Ogre none); passes through buildings (only enemies are swept). Earthquake: 35% slow + DoT at 10 Hz; slow doesn't stack with itself; Frost-ball overrides.
  - **18-Jul user delta (supersedes the earlier "lane gate"):** the Log is placeable **anywhere** in the map circle — `LogEffectSO.HasTargets` returns `true` (no enemy required). It ALWAYS rolls the fixed direction regardless of where it's dropped (was radial: outward from map center through the drop point). `rollDirection` is the single source of truth read by BOTH the sim (`LogEffectSO.Apply`) and the visual (`V4CardVfx`), so the rolling-log animation can never drift from the damage lane. Zero balance numbers touched. *(The `HasTargets` gate mentioned here was removed entirely later the same day — see §5.6.)*
  - **18-Jul user delta (fall → roll → fade + lane preview):** the Log now FALLS in from 4 units up (gravity ease-in, landing dust + 0.12 shake), THEN rolls, and at the end of `rollDistance` FADES out in place over 0.3 s (URP surface flipped to transparent for the fade only; restored opaque on pooled reuse) instead of popping off. `Card_Log.fallDelaySeconds = 0.35` so the damage sweep starts exactly when the visual lands (SpellCaster's existing delayed-Apply). While DRAGGING the Log, the ghost shows a straight **roll-lane strip** (exact `rollDistance` long × `width` wide, along `rollDirection`, near edge at the drop point) instead of the meaningless radius circle — `GhostView.ShowLane`, routed by `PlacementController.BeginDrag`; tints green/red with validity like the disc. Phases live in `V4CardVfx.LogRollView` (Falling/Rolling/Fading); all other spells unchanged.
...
- **5.6** Spells cast ANYWHERE (18-Jul user ruling, supersedes the 17-Jul "must connect with an enemy" gate): every spell — Fireball, Lightning, Earthquake, Arrows, Freeze, Log — is placeable at any point inside the map circle, enemy or not; hitting empty ground is the player's own (wasteful) choice. `PlacementValidator.IsValidSpellSpot` = map-circle check only; `SpellEffectSO.HasTargets` (and the Log's override) deleted as dead code. Verified with an empty target registry: all 6 cards valid on bare ground, still invalid outside `mapRadius` (15).
...
**18 Jul (structural):** the tower's attack range is now **DERIVED from `GameConfig.deploymentRadius`** — `kingAttack.range` is superseded/ignored. One knob moves everything together: the drawn ring, building placement, knight guard leash and the tower/mortar fire range (previously the range was a separate field manually set equal, so raising deploymentRadius grew the circle but not the tower's reach). Current tuning: deploymentRadius **7.5**. Live-verified: live range reads 7.5, a target at 6.5 is engaged (was unreachable at the old 5), a target beyond the ring is not fired upon (one in-flight shell still lands per the DT-002 determinism ruling).
